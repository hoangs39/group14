...
public class ShoppingCart1 {
    ...
    public ShoppingCart1(String id, ProductsManager store) {
        this.cartId = id;
        this.store = store;
    ...
    public void printItemsList() {
        ...
        for (Product product : items) {
            System.out.printf("Name:%s Price:%.5f QuantityInCart:%d Tax:%.5f \n", product.getName(),
                    product.getPrice(),
    ...
    // remove all the products that have the same name in the items list and then
    ...
    public boolean removeItemById(int id) {
        if (itemsList.containsKey(id)) {
            ...
            if (countItems.get(itemsList.get(id).getName()) == 0) {
                if (getAppliedCoupon().equals(itemsList.get(id).getName())) {
